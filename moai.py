...
SHELL_COMMANDS = {
    ...
    'dr.spice.cranberry': {"args": ["", "-n", "-d"], "help-info": ["", "-n", "-d"], "argc": 3}
}
if not PS:
...
def process_command(cmd, arg, arg_val):
    if   cmd == "help":
        ...
        if arg == "-d":
            drspicecranberry("drum")
        else:
            drspicecranberry("norm")
...
def drspicecranberry(mode):
    ...
    if mode == "norm":
        ...
        output_file_path = "results/" + input("\t enter an output file to create or write to: ")
        ...
        print("\t -d mode is not yet implemented. L.")
